...
import (
	"errors"
	"fmt"
	"github.com/cloudwego/thriftgo/tool/text/template/parse"
	"io"
	"reflect"
	"runtime"
	"strings"
...
func (s *state) evalField(dot reflect.Value, fieldName string, node parse.Node, args []parse.Node, final, receiver reflect.Value) reflect.Value {
	...
	if method := CacheMethodByName(ptr, fieldName); method.IsValid() {
	...
	case reflect.Struct:
		tField, ok := CacheFieldByName(receiver.Type(), fieldName)
		if ok {
	...
	case reflect.Pointer:
		...
		if etyp.Kind() == reflect.Struct {
			if _, ok := CacheFieldByName(etyp, fieldName); !ok {
